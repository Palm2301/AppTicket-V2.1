...
export const CommandCenter: React.FC<CommandCenterProps> = ({
  ...
  const avgResolutionHours = resolvedTickets.length > 0
    ? (Number(resolvedTickets.reduce((acc, curr) => acc + (curr.resolutionHours || 3.5), 0) / (resolvedTickets.length || 1)) || 3.5).toFixed(1)
    : '4.2';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-[#12141a] border border-zinc-800/80 rounded-xl p-5 shadow-sm relative overflow-hidden group hover:border-red-900/60 transition-all">
          ...
          <div className="flex items-baseline gap-3">
            <span className="text-3xl font-extrabold text-white font-mono">{criticalTickets.length || 2}</span>
            <span className="text-xs text-red-400/90 font-mono flex items-center gap-0.5">
              ↑ 1 today
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#101217] border border-zinc-800 p-4 rounded-xl flex items-center justify-between">
          <div>
            ...
            <div className="text-[10px] text-emerald-400 mt-0.5">ประสิทธิภาพเฉลี่ย 91%</div>
          </div>
          <div className="w-10 h-10 rounded-lg bg-zinc-800/80 flex items-center justify-center text-zinc-300">
              ...
              <div key={tech.id} className="p-3 bg-[#161820] rounded-lg border border-zinc-800/60 flex flex-col gap-2">
                ...
                <div className="w-full h-2 bg-zinc-800 rounded-full overflow-hidden flex">
                  <div
                    ...
                    style={{ width: `${(tech.activeTickets / 10) * 100}%` }}
                  />
                  <div
          ...
          <div>
            ...
            <div className="space-y-4 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-zinc-800">
              ...
              <div className="relative pl-6">
                <span className="absolute left-1 top-1.5 w-2.5 h-2.5 rounded-full bg-red-400 -translate-x-1/2 ring-4 ring-[#12141a]" />
                <div className="text-[11px] font-mono text-zinc-400">10:42 AM</div>
                <div className="text-xs text-zinc-200 mt-0.5 font-medium">
                  <span className="text-red-400 font-semibold">Critical:</span> Server Cluster Alpha unreachable.
                </div>
                <div className="text-[11px] text-zinc-400 mt-0.5 font-mono">
                  Ticket #XT-8942 • Auto-assigned to J. Chen
                </div>
              </div>

              {/* Event 2 */}
              <div className="relative pl-6">
                <span className="absolute left-1 top-1.5 w-2.5 h-2.5 rounded-full bg-emerald-400 -translate-x-1/2 ring-4 ring-[#12141a]" />
                <div className="text-[11px] font-mono text-zinc-400">10:15 AM</div>
                <div className="text-xs text-zinc-200 mt-0.5 font-medium">
                  Ticket resolved: Database Sync Latency.
                </div>
                <div className="text-[11px] text-zinc-400 mt-0.5 font-mono">
                  Ticket #XT-8938 • Closed by M. Kowalski
                </div>
              </div>

              {/* Event 3 */}
              <div className="relative pl-6">
                <span className="absolute left-1 top-1.5 w-2.5 h-2.5 rounded-full bg-cyan-400 -translate-x-1/2 ring-4 ring-[#12141a]" />
                <div className="text-[11px] font-mono text-zinc-400">09:55 AM</div>
                <div className="text-xs text-zinc-200 mt-0.5 font-medium">
                  New user onboarding request initiated.
                </div>
                <div className="text-[11px] text-zinc-400 mt-0.5 font-mono">
                  Ticket #XT-8941 • Unassigned
                </div>
              </div>

              {/* Event 4 */}
              <div className="relative pl-6">
                <span className="absolute left-1 top-1.5 w-2.5 h-2.5 rounded-full bg-amber-400 -translate-x-1/2 ring-4 ring-[#12141a]" />
                <div className="text-[11px] font-mono text-zinc-400">09:30 AM</div>
                <div className="text-xs text-zinc-200 mt-0.5 font-medium">
                  Transfer Form TF6908013 approved by Manager.
                </div>
                <div className="text-[11px] text-zinc-400 mt-0.5 font-mono">
                  Asset: Urovo DT50S & DJI Mic • Moving to Seacon B
                </div>
              </div>
            </div>
          </div>
